...
(() => {
  ...
  const card = film => `<article class="video-card"><a href="VIDEO.html?id=${encodeURIComponent(film.id)}"><div class="video-thumb"><img src="${thumbnail(film)}" alt="${escapeHtml(film.title)} video thumbnail" loading="lazy"><span class="play-mini">Play</span></div><p class="eyebrow">${escapeHtml(film.category)}</p><h3>${escapeHtml(film.title)}</h3><p class="deck">Watch the Video...</p></a></article>`;
  ...
  function renderDatedBrowse() {
    ...
    const render = () => { const query = search.value.trim().toLowerCase(), matches = film => (activeCategory === 'All' || film.category === activeCategory) && `${film.title} ${film.category} ${film.deck}`.toLowerCase().includes(query), today = CCC_TODAYS_VIDEOS.filter(matches), yesterday = archive.filter(matches), total = today.length + yesterday.length; grid.innerHTML = group('todays-videos-heading', 'Today&#8217;s Videos', today, 'Newly Added') + group('august-27-videos-heading', 'August 27, 2026', yesterday, 'Archive Edition'); summary.textContent = total ? 'The Cheesborough Carceral Collections Video Repository' : 'No videos shown.'; empty.hidden = Boolean(total); };
