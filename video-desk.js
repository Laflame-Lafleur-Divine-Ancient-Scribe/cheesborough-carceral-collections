...
(() => {
  ...
  const thumb = (film, loading = '') => film.embed ? `<img src="https://i.ytimg.com/vi/${encodeURIComponent(film.embed)}/hqdefault.jpg" alt="${esc(film.title)} video thumbnail" ${loading}>` : '<span class="source-poster">Source unavailable</span>';
  ...
  function browse() {
    ...
    if (count) count.textContent = `${CCC_VIDEO_CATALOG.length} films`;
    ...
    const render = () => {
      ...
      results.textContent = `${films.length} ${films.length === 1 ? 'film' : 'films'} shown.`;
  ...
  function comments(film) {
    ...
    const paint = (items = []) => {
      ...
      status.textContent = items.length ? '' : 'No comments yet. Start the conversation.';
    };
  ...
  function watch() {
    ...
    if (!film) { root.innerHTML = '<section class="not-found"><h1>Video not found</h1><p>That screening is not in the CrimeNewsTV catalog.</p><a href="VIDEOS.html">Return to CrimeNewsTV</a></section>'; return; }
    const related = CCC_VIDEO_CATALOG.filter((entry) => entry.id !== film.id).slice(0,7);
    const player = film.embed ? `<iframe src="https://www.youtube-nocookie.com/embed/${encodeURIComponent(film.embed)}?rel=0" title="${esc(film.title)}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>` : '<div class="source-poster">Video source unavailable</div>';
    root.innerHTML = `<div class="watch-shell"><article class="watch-main"><a class="back-link" href="VIDEOS.html">&larr; Back to CrimeNewsTV</a><section class="player">${player}</section><h1 class="watch-title">${esc(film.title)}</h1><p class="watch-meta">${esc(film.category)} · Curated public video</p><div class="watch-actions"><a class="action-btn" href="${link(film)}" target="_blank" rel="noreferrer">Watch on source ↗</a><a class="action-btn" href="#discussion">Join discussion</a></div><p class="watch-description">${esc(film.description)}</p><p class="embed-note">If the embedded player is unavailable, use “Watch on source” to open the original public video.</p><section class="discussion" id="discussion" aria-labelledby="discussion-heading"><div class="discussion-head"><div><h2 id="discussion-heading">Comments</h2><span id="comment-count" class="community-thread-status"></span></div><span data-community-account></span></div><p id="comment-status" class="community-thread-status" aria-live="polite"></p><form id="comment-form" class="comment-compose"><span class="avatar" aria-hidden="true">+</span><textarea id="comment-field" placeholder="Add a respectful comment" aria-label="Add a comment" required></textarea><div class="comment-actions"><button type="submit">Post comment</button></div></form><div id="comment-list"></div></section></article><aside class="up-next" aria-label="Up next"><h2>Up Next</h2>${related.map((entry) => `<a class="next-card" href="VIDEO.html?id=${encodeURIComponent(entry.id)}">${thumb(entry, 'loading="lazy"')}<div><h3>${esc(entry.title)}</h3><p>${esc(entry.category)}</p></div></a>`).join('')}</aside></div>`;
