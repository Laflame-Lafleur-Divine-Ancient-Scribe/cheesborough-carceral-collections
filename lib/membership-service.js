...
function createMembershipService({db,ensureSchema,user,stripe,json,parseBody,rate,isOwner,siteUrl,mail,env=process.env}){
  ...
  async function contentReady(){
    await ensure();
    const rows=(await db().query('SELECT resource_id FROM member_content')).rows;
    return resources.every(r=>rows.some(row=>row.resource_id===r.id));
  }
  ...
  async function handle(request,response,url){
    ...
    if(route==='/api/membership/publish'&&request.method==='POST'){
      if(!isOwner(account))return json(response,403,{error:'Only the site owner can publish member reading.'});
      await ensure();
    ...
    if(route.startsWith('/api/membership/content/')&&request.method==='GET'){
      resource=resources.find(r=>r.id===route.slice('/api/membership/content/'.length));
      if(!resource)return json(response,404,{error:'This collection could not be found.'});
      requiredTier=resource.tier;
    }else if(route==='/api/membership/export'||/^\/api\/membership\/notebook(?:\/[0-9a-f-]{36})?$/.test(route))requiredTier='legacy_circle';
    ...
    const personalRead=!resource&&['GET','DELETE'].includes(request.method);
    await ensure();
    if(!personalRead){
    ...
    if(resource){
      const content=(await db().query('SELECT sections FROM member_content WHERE resource_id=$1',[resource.id])).rows[0];
      if(!content)return json(response,503,{error:'This member reading is being prepared. Please try again shortly.'});
      return json(response,200,{resource:{...resource,sections:content.sections}});
    }
    ...
    if((request.method==='POST'&&!noteId)||(request.method==='PUT'&&noteId)){
      ...
      const result=await transaction(`member-notes:${account.id}`,async c=>{
        ...
        if(count>=200)return {limit:true};
      ...
      if(result.limit)return json(response,409,{error:'Your notebook holds up to 200 notes. Export or edit your existing notes.'});
