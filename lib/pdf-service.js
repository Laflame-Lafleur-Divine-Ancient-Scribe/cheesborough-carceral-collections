...
function createPdfService({user,state,json,rate,root=path.resolve(__dirname,'..'),cache=path.join(os.tmpdir(),'ccc-pdf-cache'),fetcher=fetch,runFile=run,documents=catalog.documents}){
 ...
 return async function handle(req,res,url){
  ...
  if(action==='download'){
   const account=await user(req);if(!account)return json(res,401,{error:'Sign in with a paid subscription to download PDFs.',requiredTier:'plugged_in'});
   const access=await state(account);if(rank(access.tier)<1)return json(res,403,{error:'PDF downloads require an active paid subscription.',requiredTier:'plugged_in'});
  }
