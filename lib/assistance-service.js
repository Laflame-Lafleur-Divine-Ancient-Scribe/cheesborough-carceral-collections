...
function createAssistanceService({user,state,json,rate,data,buildPlan,buildPacketText}){
 ...
 return async function handle(request,response,url){
  ...
  const account=await user(request);if(!account)return json(response,401,{error:'Sign in with a paid subscription to download a PDF. Reading and plain-text packets are free.',requiredTier:'plugged_in'});
  const access=await state(account);if(rank(access.tier)<1)return json(response,403,{error:'PDF downloads require an active paid subscription. You can still read and download plain text for free.',requiredTier:'plugged_in'});
