...
function createCommunityHubService(deps) {
  ...
  async function toggleConnect(request, response, targetUserId) {
    ...
    const existing = await database.query(
      'SELECT id, status FROM community_connections WHERE (requester_id = $1 AND receiver_id = $2) OR (requester_id = $2 AND receiver_id = $1)',
      [currentUser.id, targetUserId]
    );
  ...
  async function listMembers(request, response) {
    ...
    const membersRes = await database.query(`
      SELECT id, display_name, username, role, profile_about, profile_location, avatar_updated_at
      FROM community_users
      WHERE email LIKE '%@carceralcollections.internal' OR role = 'owner'
    ...
    if (currentUser) {
      ...
      connsRes.rows.forEach(r => {
        ...
        if (r.status === 'accepted') connectedIds.add(otherId);
        else if (r.status === 'pending') pendingIds.add(otherId);
      });
    }
