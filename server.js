...
async function permitResetEmail(emailHash) {
    const key = `community:reset-email:${emailHash}:${Math.floor(Date.now()/3600000)}`;
    const reply = await redisPipeline([['INCR',key],['EXPIRE',key,3605]]);
    return Boolean(reply && Number(reply[0])<=3);
}
...
async function v2StartSession(response, user) { const sid = crypto.randomBytes(32).toString('hex'), key = v2SessionKey(sid); if (!key) return communityJson(response, 503, { error: 'Community sessions are not configured yet.' }); const saved = await redisPipeline([['SET', key, JSON.stringify({ id: user.id, sessionVersion: Number(user.session_version || 0) }), 'EX', 604800]]); if (!saved) return communityJson(response, 503, { error: 'Community sessions are temporarily unavailable.' }); v2Cookie(response, sid); return communityJson(response, 201, { user: { id: user.id, displayName: user.display_name, role:user.role || 'member', avatarUpdatedAt:user.avatar_updated_at || null } }); }
async function v2Login(request, response) { applyApiCors(request, response); const db = communityDb(); if (!db || !process.env.REDIS_URL) return communityJson(response, 503, { error: 'Community accounts are not configured yet.' }); try { await ensureCommunitySchema(); } catch { return communityJson(response, 503, { error: 'Community accounts are temporarily unavailable.' }); } if (!await v2Rate(request, 'login', 12, 900)) return communityJson(response, 429, { error: 'Please wait before trying again.' }); const body = await parseCommunityBody(request), identifier = String(body?.email || body?.username || body?.login || '').trim(), password = String(body?.password || ''); if (!identifier || !password) return communityJson(response, 401, { error: 'Email/username or password is not correct.' }); const isEmail = identifier.includes('@'), normalizedEmail = isEmail ? normalizeCommunityEmail(identifier) : null; if (isEmail && !normalizedEmail) return communityJson(response, 401, { error: 'Email/username or password is not correct.' }); const accountScope = crypto.createHash('sha256').update(identifier.toLowerCase()).digest('hex').slice(0, 24); if (!await v2Rate(request, `login-account:${accountScope}`, 10, 900)) return communityJson(response, 429, { error: 'Please wait before trying again.' }); try { const result = normalizedEmail ? await db.query('SELECT id,display_name,password_hash,status,avatar_updated_at,role,session_version FROM community_users WHERE email=$1', [normalizedEmail]) : await db.query('SELECT id,display_name,password_hash,status,avatar_updated_at,role,session_version FROM community_users WHERE LOWER(display_name)=LOWER($1) OR (username IS NOT NULL AND LOWER(username)=LOWER($1))', [identifier]); const user = result.rows[0]; if (!user || user.status !== 'active' || !await argon2.verify(user.password_hash, password)) { await db.query('INSERT INTO community_audit_log (event_type,metadata) VALUES ($1,$2)', ['login_failed', JSON.stringify({ accountFound: Boolean(user) })]); return communityJson(response, 401, { error: 'Email/username or password is not correct.' }); } await db.query('UPDATE community_users SET last_login_at=now(),last_activity_at=now() WHERE id=$1', [user.id]); await db.query('INSERT INTO community_audit_log (user_id,event_type) VALUES ($1,$2)', [user.id, 'logged_in']); return v2StartSession(response, user); } catch { return communityJson(response, 503, { error: 'Community accounts are temporarily unavailable.' }); } }
async function v2OwnerRecovery(request, response) {
    ...
    try {
        ...
        const result = await db.query("UPDATE community_users SET password_hash=$1,session_version=session_version+1,status='active',role='owner' WHERE lower(email)=$2 RETURNING id,display_name,role,avatar_updated_at,session_version", [hash, ownerEmail]);
...
function v2AvatarResponse(response, status, body, type) { response.writeHead(status, { 'Content-Type': type || 'text/plain; charset=utf-8', 'Cache-Control': status === 200 ? 'public, max-age=86400, stale-while-revalidate=604800' : 'no-store', 'Access-Control-Allow-Origin': '*', 'X-Content-Type-Options': 'nosniff' }); response.end(body); }
async function v2AvatarGet(request, requestUrl, response) { applyApiCors(request, response); let id = String(requestUrl.pathname.split('/').pop() || ''); const db = communityDb(); if (!db) return v2AvatarResponse(response, 503, 'Unavailable'); if (!/^[0-9a-f-]{36}$/i.test(id)) { const user = await v2User(request); if (!user) return v2AvatarResponse(response, 404, 'Not found'); id = user.id; } try { const result = await db.query("SELECT avatar_data,avatar_mime_type FROM community_users WHERE id=$1 AND status<>'banned'", [id]); const avatar = result.rows[0]; return avatar?.avatar_data && avatar?.avatar_mime_type ? v2AvatarResponse(response, 200, avatar.avatar_data, avatar.avatar_mime_type) : v2AvatarResponse(response, 404, 'Not found'); } catch { return v2AvatarResponse(response, 503, 'Unavailable'); } }
...
const server = http.createServer((request, response) => {
    ...
    if (request.method === 'POST' && ['/api/auth/forgot-password','/api/auth/reset-password'].includes(requestUrl.pathname)) {
        applyApiCors(request,response);
        accountEmailService(request,response,requestUrl.pathname).catch(()=>communityJson(response,503,{error:'Password reset is temporarily unavailable. Please try again.'}));
        return;
    }
    ...
    if (request.method === 'GET' && (requestUrl.pathname === '/api/auth/avatar' || /^\/api\/auth\/avatar\/[0-9a-f-]{36}$/i.test(requestUrl.pathname))) {
