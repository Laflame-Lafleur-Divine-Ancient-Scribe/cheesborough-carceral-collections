(() => {
  ...
  const numericColumns = new Set(['views','visitors','sessions','page_views','average_time','average_engagement','engagement_seconds','active_seconds','pages_per_session','scroll_depth','low_engagement_rate','entrances','exits','returning_readers','member_views','anonymous_views','count','value','milestone','recent_views','previous_views','growth','delivered','opened','failed','latitude','longitude','accuracy_km']);
  const isNumericColumn = (key,value) => numericColumns.has(key) && value!=='' && value!=null && Number.isFinite(Number(value));
  const tableValue = (key,value) => isNumericColumn(key,value) ? Number(value).toLocaleString(undefined,{maximumFractionDigits:2}) : value;
  ...
  function chartHtml(chart,index) {
    const rows=window.CCCLineCharts.rowsFor(chart);
    return `<section class="report-panel chart-panel"><h2>${esc(title(chart.title))}</h2><div class="responsive-line-chart" data-chart-index="${index}">${window.CCCLineCharts.draw(chart)}</div>${rows.length?`<details class="chart-data"><summary>View exact data (${rows.length} ${rows.length===1?'row':'rows'})</summary><div class="table-scroll"><table><thead><tr><th>Label</th><th>Value</th></tr></thead><tbody>${rows.map(row=>`<tr><td>${esc(row.label || 'Unknown')}</td><td>${esc(format(Number(row.value),'decimal'))}</td></tr>`).join('')}</tbody></table></div></details>`:''}</section>`;
  }
  function metricHelpHtml(metric) {
    return `<button type="button" class="metric-help-button" data-metric-help="${esc(metric.key)}" aria-haspopup="dialog">About this metric<span class="visually-hidden">: ${esc(metric.label)}</span></button>`;
  }
  function metricExplanationHtml(metric) {
    const help=window.CCCMetricHelp.describe(metric,state.view);
    return `<dl><dt>What it measures</dt><dd>${esc(help.measures)}</dd><dt>How to use it</dt><dd>${esc(help.assessment)}</dd><dt>What is included</dt><dd>${esc(help.scope)}</dd></dl>${metric.previous!=null?'<p>Previous period uses the same filters over an equally long period immediately before the selected range. A percentage change is shown only when the previous value is greater than zero.</p>':''}${metric.value==null?'<p>No data yet means no value is available; it is not the same as a measured zero.</p>':''}`;
  }
  ...
  function bindActions(){
    window.CCCLineCharts.bind($('#dashboard-content'),report?.charts||[]);
    document.querySelectorAll('[data-metric-help]').forEach(button=>button.addEventListener('click',()=>{
      const metric=report?.metrics?.find(item=>item.key===button.dataset.metricHelp);
      if(!metric)return;
      $('#metric-dialog-title').textContent=title(metric.label);
      $('#metric-explanation').innerHTML=metricExplanationHtml(metric);
