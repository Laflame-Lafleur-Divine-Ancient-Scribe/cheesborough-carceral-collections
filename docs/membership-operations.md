...
- Public source: reading metadata in `lib/membership-catalog.js` and all six complete reading guides in `data/reading-guides.json`, intentionally public under the user's free-articles policy.
- Public reading updates: PostgreSQL `member_content`, editable through an owner-only publication endpoint. The API serves these updates publicly, with the bundled public editions available if no override exists or the database is unavailable. No manual content import is needed to launch the free readings.
- Personal notebook: PostgreSQL `member_notes`, scoped to the authenticated account on every request. Users keep read, download, and delete access after cancellation; creating/editing requires Full Member or Legacy Circle.
...
6. The six free reading guides ship with the application; no private import is required. Optional owner updates use the owner-only publication endpoint and remain public. Verify `/api/membership/catalog` and `/api/membership/content/dozier` without signing in, and check that `/api/membership` reports `configured: true`.
